...
RythmoWidget::RythmoWidget(QWidget *parent)
    ...
    , m_speed(100)
    , m_isPlaying(false)
    , m_visualStyle(Standalone)
    , m_fontSize(16)
    , m_verticalPadding(4)
    , m_textColor(QColor(34, 34, 34))
    , m_barColor(QColor(0, 0, 0, 0))
    , m_playingBarColor(QColor(0, 0, 0, 0))
    , m_lastMouseX(0)
    , m_cachedCharWidth(-1)
...
QString RythmoWidget::text() const
{
    return m_text;
}
...
{
    emit seekRequested(m_pendingSeekPosition);
}
...
void RythmoWidget::paintEvent(QPaintEvent *event)
{
    ...
    double pixelOffset = (static_cast<double>(m_currentPosition) / 1000.0) * m_speed;
    ...
    if (cw > 0 && !m_text.isEmpty()) {
        ...
        int lastVisibleIdx = std::min(m_text.length(),
    ...
    if (cw > 0) {
        ...
        bool drawHandle = (m_visualStyle == Standalone || m_visualStyle == UnifiedTop);
        bool drawLabel = (m_visualStyle == Standalone || m_visualStyle == UnifiedTop);
        
        int lineTop = bandY;
        int lineBottom = bandY + bandHeight;
        
        if (m_visualStyle == UnifiedTop) lineBottom += 2;
        if (m_visualStyle == UnifiedBottom) lineTop -= 2;
...
void RythmoWidget::keyPressEvent(QKeyEvent *event)
{
    ...
    if (!event->text().isEmpty() && event->text().at(0).isPrint()) {
        emit characterTyped(event->text());
    }
}
