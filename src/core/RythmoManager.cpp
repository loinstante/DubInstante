...
void RythmoManager::insertCharacter(int trackIndex, const QString &character)
{
    ...
    while (trackText.length() < idx) {
        trackText.append(' ');
    }
    
    trackText.insert(idx, character);
...
void RythmoManager::deleteCharacter(int trackIndex, bool before)
{
    ...
    if (before) {
        // Backspace behavior
        if (idx > 0 && idx <= trackText.length()) {
            trackText.remove(idx - 1, 1);
            emit textChanged(trackIndex, trackText);
        }
    } else {
        // Delete behavior
        if (idx >= 0 && idx < trackText.length()) {
            trackText.remove(idx, 1);
            emit textChanged(trackIndex, trackText);
        }
    }
}

int RythmoManager::trackCount() const
{
...
qint64 RythmoManager::currentPosition() const
{
    return m_currentPosition;
}

void RythmoManager::invalidateFontCache()
{
    m_cachedCharWidth = -1;
}
...
void RythmoManager::requestSeek(int trackIndex, int deltaPixels)
{
    ...
    qint64 newPosition = std::max(qint64(0), 
                                   m_currentPosition + static_cast<qint64>(timeDeltaMs));
